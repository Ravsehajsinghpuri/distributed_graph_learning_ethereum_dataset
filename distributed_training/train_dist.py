...
class DistSAGE(nn.Module):
    ...
    def inference(self, g, x, batch_size, device):
        ...
        y = dgl.distributed.DistTensor((g.number_of_nodes(), self.n_hidden), th.float64, 'h',
                                       persistent=True)
        for l, layer in enumerate(self.layers):
            if l == len(self.layers) - 1:
                y = dgl.distributed.DistTensor((g.number_of_nodes(), self.n_classes),
                                               th.float64, 'h_last', persistent=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--standalone', default=False, action='store_true', help='run in the standalone mode')
