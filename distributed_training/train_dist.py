...
class DistSAGE(nn.Module):
    ...
    def inference(self, g, x, batch_size, device):
        ...
        for l, layer in enumerate(self.layers):
            if l == len(self.layers) - 1:
                y = dgl.distributed.DistTensor((g.number_of_nodes(), 1),
...
def run(args, device, data):
    # Unpack data
    train_nid, val_nid, in_feats, g = data
    shuffle = True
    # Create sampler
    ...
    model = DistSAGE(in_feats, args.num_hidden, args.num_layers, F.relu, args.dropout)
    ...
    for epoch in range(args.num_epochs):
        ...
        if epoch % args.eval_every == 0 and epoch != 0:
            start = time.time()
            val_acc= evaluate(model.module, g, g.ndata['feat'],
                                         g.ndata['labels'], val_nid, args.batch_size_eval, device)
            print('Part {}, Val Acc {:.4f}, time: {:.4f}'.format(g.rank(), val_acc,time.time() - start))
...
def main(args):
    ...
    print('part {}, train: {} (local: {}), val: {} (local: {})'.format(
    ...
    data = train_nid, val_nid, in_feats, g
...
if __name__ == '__main__':
    ...
    parser.add_argument('--graph_name', type=str, help='graph name',default='partitioned_graph_1M')
